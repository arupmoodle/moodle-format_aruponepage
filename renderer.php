...
class format_aruponepage_renderer extends format_section_renderer_base {
    ...
    protected function start_section_list() {
        return html_writer::start_tag('ul', array('class' => 'aruptopics '));
    }
    ...
    protected function section_availability_message($section, $canviewhidden) {
        ...
        $o = '';
        if (!$section->uservisible) {
            $o .= html_writer::tag('i', '', array(
                    'class' => 'availabilityinfo fa fa-lock',
                    'data-toggle' => 'tooltip',
                    'title' => $section->availableinfo
                ));
            ...
            if ($fullinfo) {
                $info = get_string(
                        ...
                        'condition', $fullinfo);
                $o .= html_writer::tag('i', '', array(
                    'class' => 'availabilityinfo fa fa-lock'.($section->showavailability == 0?' lockinvisible':''),
                    'data-toggle' => 'tooltip',
                    'title' => $info
                ));
            }
        }
        // If we get this far, conditions met, if there were ever any then show unlocked icon
        if (empty($o) && (!empty($section->conditionscompletion)||!empty($section->conditionsfield)||!empty($section->conditionsgrade))) {
            return '<i class="fa fa-unlock availabilityinfo '.($section->showavailability == 0?' lockinvisible':'').'"></i>';
        }
        return $o;
    }
